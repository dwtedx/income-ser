...
public interface IDiIncomeMapper {
	...
	*/
	...
	"</when>",
	"<when test='role != null and role != 0 and role != \"\"'>",
	...
	*/
	@Select({"<script>",
	"select count(*) from di_income i where 1 = 1",
	"and (i.deletefalag = 0 or i.deletefalag is null)",
	"<when test='userid != null and userid != \"\"'>",
	"and i.userid = #{userid,jdbcType=INTEGER}",
	"</when>",
	...
	"order by id desc;",
	"</script>"})
	int selectByExpExcelsPool(@Param("userid")Integer userid, @Param("role")Integer role, @Param("moneysumstart")BigDecimal moneysumstart, @Param("moneysumend")BigDecimal moneysumend, 
			@Param("typeid")String[] typeid, @Param("accountid")String[] accountid, @Param("recordtimestart")String recordtimestart, @Param("recordtimeend")String recordtimeend);
